...
<?php } if ($this->_rootref['S_NO_DISPLAY_BOOKMARKS']) {  ?>
	...
	<tr>
		<td class="row1" colspan="4" align="center"><b class="genmed"><?php echo ((isset($this->_rootref['L_BOOKMARKS_DISABLED'])) ? $this->_rootref['L_BOOKMARKS_DISABLED'] : ((isset($user->lang['BOOKMARKS_DISABLED'])) ? $user->lang['BOOKMARKS_DISABLED'] : '{ BOOKMARKS_DISABLED }')); ?></b></td>
	</tr>
<?php } else { if ($this->_rootref['TOTAL_TOPICS']) {  ?>
	...
	<?php } $_topicrow_count = (isset($this->_tpldata['topicrow'])) ? sizeof($this->_tpldata['topicrow']) : 0;if ($_topicrow_count) {for ($_topicrow_i = 0; $_topicrow_i < $_topicrow_count; ++$_topicrow_i){$_topicrow_val = &$this->_tpldata['topicrow'][$_topicrow_i]; if (!($_topicrow_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
		...
		<?php if ($_topicrow_val['S_DELETED_TOPIC']) {  ?>
			...
			<td class="postdetails row" style="padding: 4px" width="100%" colspan="2"><?php echo ((isset($this->_rootref['L_DELETED_TOPIC'])) ? $this->_rootref['L_DELETED_TOPIC'] : ((isset($user->lang['DELETED_TOPIC'])) ? $user->lang['DELETED_TOPIC'] : '{ DELETED_TOPIC }')); ?></td>
		<?php } else { ?>
	...
	<?php }} else { ?>
		...
		<tr>
			<td class="row1" colspan="4" align="center"><b class="genmed"><?php echo ((isset($this->_rootref['L_NO_BOOKMARKS'])) ? $this->_rootref['L_NO_BOOKMARKS'] : ((isset($user->lang['NO_BOOKMARKS'])) ? $user->lang['NO_BOOKMARKS'] : '{ NO_BOOKMARKS }')); ?></b></td>
		</tr>
	<?php } if (sizeof($this->_tpldata['topicrow'])) {  ?>
