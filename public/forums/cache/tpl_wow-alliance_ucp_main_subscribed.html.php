...
<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP2_START'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP2_START'] : ''; echo ((isset($this->_rootref['L_UCP'])) ? $this->_rootref['L_UCP'] : ((isset($user->lang['UCP'])) ? $user->lang['UCP'] : '{ UCP }')); echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP2_END'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP2_END'] : ''; ?>
...
<?php if ($this->_rootref['S_FORUM_NOTIFY']) {  ?>
	...
	<?php $_forumrow_count = (isset($this->_tpldata['forumrow'])) ? sizeof($this->_tpldata['forumrow']) : 0;if ($_forumrow_count) {for ($_forumrow_i = 0; $_forumrow_i < $_forumrow_count; ++$_forumrow_i){$_forumrow_val = &$this->_tpldata['forumrow'][$_forumrow_i]; if (!($_forumrow_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
			...
			<td class="row" style="padding: 4px;" width="20" align="center" valign="middle"><?php echo $_forumrow_val['FORUM_FOLDER_IMG']; ?></td>
			<td class="row" style="padding: 4px;" width="100%"><p class="topictitle"><a href="<?php echo $_forumrow_val['U_VIEWFORUM']; ?>"><?php echo $_forumrow_val['FORUM_NAME']; ?></a></p></td>
			<td class="gensmall row" style="padding: 4px;" align="center" valign="middle" nowrap="nowrap"><?php if ($_forumrow_val['LAST_POST_TIME']) {  echo $_forumrow_val['LAST_POST_TIME']; ?><br /><?php echo $_forumrow_val['LAST_POST_AUTHOR_FULL']; ?> <a href="<?php echo $_forumrow_val['U_LAST_POST']; ?>"><?php echo (isset($this->_rootref['LAST_POST_IMG'])) ? $this->_rootref['LAST_POST_IMG'] : ''; ?></a><?php } else { echo ((isset($this->_rootref['L_NO_POSTS'])) ? $this->_rootref['L_NO_POSTS'] : ((isset($user->lang['NO_POSTS'])) ? $user->lang['NO_POSTS'] : '{ NO_POSTS }')); } ?></td>
			<td style="padding: 4px;"> <input type="checkbox" class="radio" name="f[<?php echo $_forumrow_val['FORUM_ID']; ?>]" /> </td>
		</tr>
	<?php }} else { ?>

		<tr>
			<td class="row1" colspan="4" align="center"><b class="genmed"><?php echo ((isset($this->_rootref['L_NO_WATCHED_FORUMS'])) ? $this->_rootref['L_NO_WATCHED_FORUMS'] : ((isset($user->lang['NO_WATCHED_FORUMS'])) ? $user->lang['NO_WATCHED_FORUMS'] : '{ NO_WATCHED_FORUMS }')); ?></b></td>
		</tr>
	...
	<?php } $_topicrow_count = (isset($this->_tpldata['topicrow'])) ? sizeof($this->_tpldata['topicrow']) : 0;if ($_topicrow_count) {for ($_topicrow_i = 0; $_topicrow_i < $_topicrow_count; ++$_topicrow_i){$_topicrow_val = &$this->_tpldata['topicrow'][$_topicrow_i]; if (!($_topicrow_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
			...
			<td class="row" style="padding: 4px;" width="20" align="center" valign="middle"><?php echo $_topicrow_val['TOPIC_FOLDER_IMG']; ?></td>
			<td class="row" style="padding: 4px;" width="100%" valign="top">
			...
			<td class="row" style="padding: 4px;" align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>" valign="top" nowrap="nowrap">
			...
			<td class="row" style="padding: 4px;"> <input type="checkbox" class="radio" name="t[<?php echo $_topicrow_val['TOPIC_ID']; ?>]" /> </td>
		</tr>
	<?php }} else { ?>

		<tr>
			<td class="row1" colspan="4" align="center"><b class="genmed"><?php echo ((isset($this->_rootref['L_NO_WATCHED_TOPICS'])) ? $this->_rootref['L_NO_WATCHED_TOPICS'] : ((isset($user->lang['NO_WATCHED_TOPICS'])) ? $user->lang['NO_WATCHED_TOPICS'] : '{ NO_WATCHED_TOPICS }')); ?></b></td>
		</tr>
